   ...
   function displayItems(id, write,cpc) {
         ...
         snapshot.forEach(function(childSnapshot) {
           ...
           <div class="view-count-item">
           	...
           	<span id="day" class="view-count-number">${item.count||0}</span>
           		<span id="day" class="view-count-number">= $ ${item.count*cpc}</span>
           </div>
   ...
   function custom_path(path,write,text,cpc){
		...
		if (data) {
			document.getElementById(write).innerHTML = `
			           <div class="view-count-item">
			           	...
			           	<span id="day" class="view-count-number">= $ ${data.count*cpc}</span>
			           </div>
			`;
		}
	});
